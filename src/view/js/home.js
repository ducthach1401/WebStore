...
function showItem(item, row, count) {
  ...
  temp += '<p class="card-text" id="gt{}">Giá tiền:<b> {} </b> </p>'
    .replace("{}", item._id)
    .replace("{}", item.cost.toLocaleString("vi-vn"));
  temp += '<p class="card-text">Đã bán: {}</p>'.replace(
    "{}",
    item.quantity_purchased
  );
  temp +=
    '<p class="card-text">Thành tiền: <input type="text" id = "tt{}" disabled class="w-50"> </span> </p>'.replace(
      "{}",
      item._id
    );
  temp +=
...
async function cart() {
  ...
  bodyOrder.innerHTML =
    "<tr><th>STT</th><th>Tên sản phẩm</th><th>Số lượng</th><th>Đơn giá</th><th>Thành tiền</th></tr>";
  ...
  for (let item of data) {
    ...
    td = document.createElement("td");
    td.innerHTML =
      '<button class="btn btn-outline-danger" onclick="deleteItem(this.value);" value="{}">Xóa</button>'.replace(
        "{}",
        item
      );
    row.append(td);
...
async function total(number_id) {
  ...
  let cost = document
    ...
    .join("");
  cost = parseInt(cost);
  document.getElementById("tt" + id).value = (cost * quantity).toLocaleString(
    "vi-vn"
  );
}
